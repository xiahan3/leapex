...
>
> **状态贯通主线**：票据/流水 `解析(staged→parsing→success/failed/edited)` → 对账 `配对(match/partial/pending/unmatched/orphan/annotated)` → 分录 `(pending→confirmed，借贷平衡为前提；ignored/reversed)` → 试算 `(balanced/unbalanced)` → 报表 `(draft/generated)` → 账期 `(open/locked)`。
...
- 损益表（I/X 推导）、资产负债表（A/L/E 推导 + 平衡校验）、现金流量（可选）。关账（写 SYSTEM_PERIOD_LOCKED）、导出。规则见 [[财务报告生成规则与说明]] §三。
- **状态说明**：
...
> 增值与运营能力（AI 审计报告、平台管理、账号安全、账簿浏览、接入点管理）见 **[[EasybookX_产品需求文档_PRD_v1.1]]**。
