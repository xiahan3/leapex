# Leapex TCSP 公司秘书小程序 — MVP 需求文档（产品/技术版）

> **定位**：基于客户 V2.2 需求文档的 MVP 实现版，聚焦微信小程序客户端主线
> **更新**：2026-06-11
> **来源**：6.11-Leapex_TCSP_需求文档.md（V2.2）
> **作者**：产品 + HK 财务/合规理解整合
...
Leapex 是一个多产品线的香港中小企业服务平台。本小程序属于 **Part B**：
...
> **但两者在底层共享**：① 多租户/RBAC（见 Leapex v3.0/v3.1）② 账单支付模型（见 Leapex v3.2，与本文档第 12 章 1:1 映射完全一致）③ 7 年合规存档与审计日志。

### 0.2 复用决策（避免重复造轮子）

| 能力 | 来源 | 复用方式 |
|---|---|---|
| 账单=支付单元、1:1、回单上传+人工确认 | Leapex v3.2 | 直接复用 Payment/Invoice 状态机，仅业务字段不同 |
| 收款账户配置 | Leapex v3.2 | 共用 PaymentAccount 表 |
| RBAC 6 角色 | Leapex v3.1 + V2.2 第13章 | 合并角色字典（新增 Compliance/Channel Admin） |
| 审计日志 7 年 | Leapex v3.1 | 共用 AuditLog，扩展事件类型 |
| 文件加密存储/PDPO | Leapex v3.1 | 共用 |
...
| 2 地址与股本 | 注册地址 + 股本 | Leapex 虚拟地址/自选；默认每股 HKD 1.00 |
| 3 确认章程 | 章程选择 | Sample A/B/自定义（智能推荐） |
| 4 董事与秘书 | 董事 + 秘书 | 董事 KYC 复用；秘书=Leapex 内嵌 |
| 5 创办成员 | 股东/股份分配 | KYC 复用 |

### 3.4 账单与支付（复用 Leapex v3.2，对齐 V2.2 第12章）
...
| 登录 | wx.login → code 换 openid → 绑定 Leapex 账号 → JWT |
...
## 七、数据模型（TCSP 增量，与 Leapex 共表标注）

```python
class Customer:          # 小程序终端客户（与 Leapex User 可打通）
